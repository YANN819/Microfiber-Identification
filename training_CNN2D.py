...
class SpectrumModel(pl.LightningModule):
    def __init__(self, in_channel, out_class, dropout = 0.2):
        ...
        self.model = CNN2D(in_channel, out_class, dropout)
    ...
    def shared_evaluation_epoch_end(self, outputs, prefix):
        ...
        precision = precision_score(labels.cpu().numpy(), torch.argmax(preds_probs, dim=1).cpu().numpy(), average="weighted")
        recall = recall_score(labels.cpu().numpy(), torch.argmax(preds_probs, dim=1).cpu().numpy(), average="weighted")
        f1 = f1_score(labels.cpu().numpy(), torch.argmax(preds_probs, dim=1).cpu().numpy(), average="weighted")
        ...
        output_file = "roc_curve_CNN2D.csv"
        roc_data = pd.DataFrame({'FPR': fpr, 'TPR': tpr})
        roc_data.to_csv(output_file, index=False)

        # Save evaluation metrics to csv file
        metrics_file = f"{prefix}_metrics_CNN2D.csv"
        metrics_data = {
        'Accuracy': [accuracy],
        'Precision': [precision],
        'Recall': [recall],
        ...
        self.show_confusion_matrix(cm, f"{prefix}_confusion_matrix_2D.svg")
        self.show_roc_curve(fpr, tpr, roc_auc, f"{prefix}_roc_curve_2D.svg")
...
def train(args):
    ...
    full_dataset = SpectrumDataset2D(root_dir = args.dataset, transform = args.transform)
    ...
    logger = TensorBoardLogger("logs", name = "spectrum_experiment_2D")
    
    checkpoint_callback = ModelCheckpoint(dirpath = 'checkpoints_2D', filename = 'best_model_{epoch:02d}-{val_loss:.2f}', monitor = 'val_loss', mode = 'min')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default = r'D:\Anaconda3\pythonwork\ramanspec_file\data_training\cnn\spectra_images')
    parser.add_argument("--transform", default = transforms.Grayscale())
